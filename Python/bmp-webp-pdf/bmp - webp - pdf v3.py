...
class Config:
    ...
    # Поддерживаемые расширения изображений (включая TIF/TIFF)
...
def create_pdfs(processed_paths: List[Path], output_dir: Path, config: Config, logger: logging.Logger) -> None:
    ...
    Пропускает варианты, которые превышают лимит размера.
    """
    ...
    for quality, label in config.QUALITY_LEVELS:
        pdf_filename = output_dir / f"book_{label}.pdf"  # Имя выходного PDF

        # Оценка размера PDF
        est_size = estimate_pdf_size(processed_paths, quality, config.MAX_PDF_BYTES)

        # Пропуск, если размер превышает лимит (100 МБ)
        if est_size is None:
            logger.warning(
...
def main() -> None:
    ...
    # Создание каталога для результатов (подкаталог в выбранной папке)
    output_dir = input_dir / config.OUTPUT_SUBDIR
    output_dir.mkdir(parents=True, exist_ok=True)  # Создание если не существует
    logger.info(f"Каталог для результатов: {output_dir}")
...
if __name__ == "__main__":
    ...
    Обрабатывает исключения для корректного завершения:
